...
let write_r(args: string) =
    ...
    with
    | e when e.GetType() = typeof<System.UnauthorizedAccessException> -> printfn "Access denied"
...
let help_r(args:string) =
    ...
    "| exscript (file name)";
    "| cmdinfo";
    "| random (first value) [second value]"
    ]
...
let sysinfo_r(args:string) =
    ...
    let sysinf = [
        ...
        $"| Uptime: {MathF.Round(float32(((double)Environment.TickCount64/double(1060)/double(60)/double(60))), 1)}h"
        "UniShell info:"
        "| UniShell Version: Lite 1.1"
    ...
    0
let themeswitcher_r(args:string) =
    let mutable background : ConsoleColor = ConsoleColor.Black
    let mutable foreground : ConsoleColor = ConsoleColor.White
    ...
    Console.Clear()
    0
let randomnum_r(args:string) =
    let mutable max = 0;
    let mutable min = 0;
    
    if args.Split().Length = 0 then
        printfn "No arguments provided"
    elif args.Split().Length = 1 then
        
        try
            max<-Int32.Parse(args.Split()[0])
            printfn $"Number: {Random().Next(min, max)}"
        with
        | _ -> printfn "Non number"
    else
        try
            min<-Int32.Parse(args.Split()[0])
            max<-Int32.Parse(args.Split()[1])
            printfn $"Number: {Random().Next(min, max)}"
        with
        | x when x.GetType() = typeof<System.ArgumentOutOfRangeException> -> printfn "First number is greater than second number"
        | _ -> printfn $"Non numbers"
    0
...
cmds.Add ("theme", {Description = "Switches themes (Light and Dark)"; Execute = themeswitcher_r })
cmds.Add ("random", {Description = "Gives random number in range"; Execute = randomnum_r })
