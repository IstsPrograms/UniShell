...
{
    Sys.Sys ss = new Sys.Sys("UniShell", "Public 1.0.0", "UniShellCore 1.0.0", "No updates");
    string npath = path;
        ...
        switch (action.Split()[0])
        {
            ...
            case "tcpget":
                try
                {
                    ...
                    string encoding = action.Split()[2];
                    string ip = url.Split(":")[0];
                    int port = System.Convert.ToInt32(url.Split(":")[1]);
                    string saveTo = npath + action.Split()[3];
                    System.Net.Sockets.TcpClient getter = new(ip, port);
                    byte[] data = new byte[256];
                    int bytes = getter.GetStream().Read(data, 0, data.Length);
                    getter.GetStream().Close();
                    getter.Close();
                    string gettedData = String.Empty;
                    switch (encoding.ToUpper())
                    {
                        case "ASCII":
                            gettedData = System.Text.Encoding.ASCII.GetString(data, 0, bytes);
                            break;
                        case "UTF-8":
                            gettedData = System.Text.Encoding.UTF8.GetString(data, 0, bytes);
                            break;
                        case "UTF-7":
                            gettedData = System.Text.Encoding.UTF7.GetString(data, 0, bytes);
                            break;
                        case "UTF-32":
                            gettedData = System.Text.Encoding.UTF32.GetString(data, 0, bytes);
                            break;
                        case "UNICODE":
                            gettedData = System.Text.Encoding.Unicode.GetString(data, 0, bytes);
                            break;
                    }
                    System.IO.File.WriteAllText(saveTo, gettedData);
                }
                catch
                {
