...
public class SystemInfo : IQuantCommand
{
    ...
    {
        int countOfPkg = 0;
        var cmds = JsonSerializer.Deserialize(arg[2], typeof(List<string>),
            new JsonSerializerOptions() { IncludeFields = true }) as List<string>;
        
        foreach (var t in cmds)
        {
            if (t == $"{typeof(ICustomQuantCommand)}")
            {
                countOfPkg++;
            }
        }
        Engine.OutputList(new List<string>()
        {
            ...
            "UniShell info:",
            "| UniShell Version: 3.1",
            "| HydriumLite Version: 1.3",
            "| Quant.cs Version: Modified 1.0 Pre-release",
            $"| Number of common commands: {cmds.Count-countOfPkg}",
            $"| Number of custom commands: {countOfPkg}",
            $"| Number of all commands: {cmds.Count}"
        });
    }
}
